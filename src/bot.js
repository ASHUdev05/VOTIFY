...
client.once('ready', () => {
    ...
    var activities = [ `${client.guilds.cache.size} servers`, `${client.users.cache.size} users!`, `v0.0.3`, 'WATCHING your polls!' ], i = 0;
            ...
            for (const poll of pollArray) {
                if (Date.now() >= Number(poll.expiryDate)) {
                    ...
                    if ((resultsArr[0][1] == resultsArr[1][1]) && (resultsArr1[0][1] == resultsArr1[1][1])) {
                        ...
                        channel.send(`The winner of the poll was option ${resultsArr1[0][0]} \nhttps://discordapp.com/channels/${msg.guild.id}/${channel.id}/${msg.id}`);
                    }
